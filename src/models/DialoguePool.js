...
class DialogueService {
  constructor() {
    this.dialoguePool = new DialoguePool();
  ...
  getConfidenceMessage(confidence) {
    const category = confidence <= 0.3 ? 'low' : 
                    confidence <= 0.6 ? 'medium' : 'high';
    const messages = this.dialoguePool.confidenceMessages[category];
    return messages[Math.floor(Math.random() * messages.length)];
  }

  getUniqueMessage(category) {
    const available = this.dialoguePool.dialoguePool[category].filter(
